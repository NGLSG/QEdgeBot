...
async def submit(prompt) -> str:
    try:
        ...
        return res['item']['messages'][1]['text']
...
def chat(msg, sessionid, uid="", isgroup=False):
    ...
    try:
        ...
        if '重置会话' == msg.strip():
            ...
            result = loop.run_until_complete(chatbot.reset())
            loop.close()
...
def ask(prompt):
    try:
        ...
        resp = loop.run_until_complete(submit(prompt))  # 在事件循环中运行协程
        # loop.close()
...
def CreateConversion():
    try:
        ...
        with open("config.json", "r", encoding='utf-8') as jsonfile:
            config_data = json.load(jsonfile)
            chatbot = EdgeGPT.Chatbot(cookiePath=config_data['account']['cookies_path'])
    except Exception as e:
        print(e)
